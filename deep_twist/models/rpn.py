...
class DeepGrasp(nn.Module):
    def __init__(self, sizes=[2, 4, 8], aspects=[0.5, 1, 2]):
    ...
    def get_proposals(self, rpn_cls, rpn_reg):
        print(type(rpn_cls))
        grasps = (rpn_cls[:, ::2, :, :] < rpn_cls[:, 1::2, :, :]).nonzero()
        print(type(grasps))
        proposals = pytorch.zeros()
        for i in range(rpn_cls.size(0)):
            ex_props = []
            grasp_index = is_grasp[i, :, :, :].nonzero()
...
class DeepTwist(nn.Module):  # TODO: change from copy of above to add angle props
    def __init__(self, sizes=[2, 4, 8], aspects=[0.5, 1, 2]):
        super(DeepGrasp, self).__init__()
        self.sizes = sizes
        self.aspects = aspects
